...
import type { RoleStatus } from './type'

export function RoleFilterUi({ className, ...props }: React.ComponentProps<'div'>) {
  return <div className={cn('flex flex-wrap items-center gap-4', className)} {...props} />
}

RoleFilterUi.RowPerPage = function RowPerPage({
  value,
  onChange,
}: {
  value: number
...
RoleFilterUi.Status = function Status({
  value,
  onChange,
  statusData,
}: {
...
RoleFilterUi.DatePicker = function DatePicker({
  from,
  to,
  onChange,
}: {
...
RoleFilterUi.Search = function Search({
  value,
  onChange,
}: {
  value?: string
